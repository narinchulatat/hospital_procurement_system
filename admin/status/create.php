...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif ($status->checkNameExists($status->name)) {
    ...
    if (empty($errors)) {
        if ($status->create()) {
            // Log activity
            logActivity($_SESSION['user_id'], 'CREATE', 'สร้างสถานะ: ' . $status->name, 'statuses', $status->id);
            
            $_SESSION['success'] = 'เพิ่มสถานะเรียบร้อยแล้ว';
            header('Location: index.php');
            exit();
        } else {
            $_SESSION['error'] = 'เกิดข้อผิดพลาดในการเพิ่มสถานะ';
        }
    }
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0">เพิ่มสถานะ</h1>
    ...
    <div class="card-body">
        <form method="POST" data-validate>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="sort_order" name="sort_order" value="<?php echo htmlspecialchars($_POST['sort_order'] ?? ''); ?>" min="1" required>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" id="description" name="description" rows="3" placeholder="อธิบายรายละเอียดของสถานะนี้"><?php echo htmlspecialchars($_POST['description'] ?? ''); ?></textarea>
            </div>
